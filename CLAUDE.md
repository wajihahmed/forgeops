...
- **Gitea `postStart` hook must run as the `git` user, not root** — the pod has no `runAsUser`, so `postStart` runs as root by default, but the `gitea` CLI refuses to run as root (`Gitea is not supposed to be run as root`). The hook is wrapped as `su git -c 'gitea admin user create ...'`. If this regresses, admin user creation silently fails (masked by `2>/dev/null || true`) and the `gitea-seed` job crash-loops with every Gitea API call returning `401 Unauthorized`.
- **DS `:latest` tag can cause a binary/config version mismatch** — if the image tag moves between an early bootstrap and a later pod restart, DS logs `The PingDS binary version 'X' does not match the installed configuration version 'Y'` and crash-loops. Not recoverable in place; wipe the `ds-cts`/`ds-idrepo` PVCs and redeploy (same recovery procedure as the empty-password case — secrets are reused so passwords don't change). This applies whether DS is deployed via `bin/forgeops apply` or raw `kubectl apply -k` — `forgeops apply` is a thin wrapper that runs `kubectl apply -k` against the same `kustomize/overlay/default/` tree (see `bin/commands/apply`), so both paths render the identical `:latest`-pinned manifest.
- **idm-resource-server OAuth2 client secret can be fixed via AM's REST API instead of the console** — the Kubernetes secret (`amster-env-secrets`/`IDM_RS_CLIENT_SECRET`) already holds the value IDM expects; push it into AM with a authenticate → GET → flatten grouped config → PUT flow. AM's `PUT` requires a flat attribute object (the grouped shape from `GET` — `coreOAuth2ClientConfig`, `advancedOAuth2ClientConfig`, etc. — gets rejected with `Invalid attribute specified.`). The response always echoes `userpassword: null` even on success, since AM never returns secret values — verify by testing auth instead (wrong secret → `invalid_client`; correct secret against a disallowed grant type → `unauthorized_client`, which confirms auth passed). See `.claude/commands/deploy-mock-tenant.md` Step 10 for the full script.
